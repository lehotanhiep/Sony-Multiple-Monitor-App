...
private fun playback9209():ByteArray {
    // D208 AUINT32: default=[] current=[ENTER 0x00050000, PLAYBACK 0x00110000]
    val d208 = s16(0xD208)+s16(0x4006)+byteArrayOf(0x00,0x01)+
        s32(0)+s32(2)+s32(0x00050000)+s32(0x00110000)+byteArrayOf(0)
...
fun main(){
    ...
    check(logs.any{it.contains("D208 raw=0x50000")}) { logs.joinToString("\n") }
    println("PTP ZV-E10M2 PLAYBACK CENTER PASS: advertised E021/E022 bypassed -> D309 ENTER 0x0005")
}
