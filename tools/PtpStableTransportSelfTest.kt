...
fun main() {
    ...
    val worker = thread(name = "fake-zv1-stable") {
        ...
        p = readPacket(ci)
        check(op(p) == 0x1009 && tx(p) == 19 && param(p,0) == -16382)
        ...
        dataIn(co, 19, dataset, declared = 0xffffffffL); response(co, 19)
    ...
    val jpeg = controller.getLiveViewJpeg()
    ...
    println("Sony Multiple Monitor V1.0.0 stable PTP/IP + SDIO + FFFFC002 self-test OK")
}
